...
# Por defecto: intentamos cargar TensorFlow. Si no está, solo se desactiva la parte de gestos.
TF_AVAILABLE = False
TM_AVAILABLE = False
...
@st.cache_resource
def load_tm_model():
    """Carga modelo de gestos (requiere TensorFlow y gestos.h5 en la raíz)."""
    if not TF_AVAILABLE:
        return None
    try:
...
# --------- PÁGINA 3: GESTOS ---------
    ...
    if not TM_AVAILABLE:
        ...
        st.markdown(
            """
        Para usar gestos necesitas:

        1. Tener `tensorflow-cpu` en `requirements.txt`
        2. Subir el archivo `gestos.h5` a la raíz del proyecto
        """
        )
